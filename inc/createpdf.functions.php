...
function CreatePDF($hostarray) {
	global $hostManage, $stime, $timeperiod, $tmp_pdf_data, $z_tmpimg_path, $debug;
	$i = 0;
	...
	foreach($hostarray as $key=>$host) {
		...
		if($hostname == "IFS-APP"){
			$hostManage = array("556","557","566","567","571","579","581","610","611","613","614","625","627","628","629");
			$avail = 1;
		}
		elseif($hostname == "IFS-DB"){
			$hostManage = array("556","557","566","567","571","579","581","610","611","613","614","625","627","628","629");
			$avail = 1;
		}
		...
		foreach($hostGraphs as $graphkey=>$graphs) {
			...
			if($avail == 1){
			    for($i = 0;$i <= sizeof($hostManage);$i++){
				if($graphid == $hostManage[$i]){
				    $image_file = $z_tmpimg_path ."/".$trimmed_hostname ."_" .$graphid .".png";
